...
from ml.finance.datastruct import Deal, Direction, Order, OrderStatus, Period, TradeSide
from ml import util
...
class QCTest(unittest.TestCase):
    ...
    def test_parse_indicators(self):
        ...
        self.assertEquals(rst[0]["info"], {
            'type': 'SMA',
            'arguments': '60',
            'symbol': 'SPY',
            'period': Period.Minute
        })
        self.assertEquals(len(rst[0]["values"]), 390)
        self.assertEquals(rst[1]["name"], "SMA10")
        self.assertEquals(rst[1]["info"], {
            'type': 'SMA',
            'arguments': '10',
            'symbol': 'SPY',
            'period': Period.Minute
        })
        self.assertEquals(len(rst[1]["values"]), 390)

    def test_parse_result_tradebar(self):
        file_path = os.path.join(util.data_path, "CustomIndicatorAlgorithm.json")
        self.result = ResultParser()
        self.result.read_data(file_path)

        bars = self.result.get_market_data("TradeBar")
